...
export default function WakeUp({ serverTime }) {
    ...
    if (!isLate && points > 0) {
        confetti({ particleCount: 100, angle: 60, spread: 55, origin: { x: 0 } }); // Dari Kiri
        confetti({ particleCount: 100, angle: 120, spread: 55, origin: { x: 1 } });
    }
    Swal.fire({
        title: isLate ? 'MISI SELESAI!' : 'MISI BERHASIL!',
        html: `
            <div class="flex flex-col items-center gap-4 py-4" style="font-family: 'Inter', sans-serif;">
                <div class="flex gap-4">
                    <div class="bg-yellow-50 p-4 rounded-[2rem] border-2 border-yellow-200 flex flex-col items-center min-w-[90px]">
                        ...
                        <p class="text-xl font-black text-slate-800">+${points}</p>
                    </div>
                ...
                <p class="text-sm text-slate-500 font-medium px-4 text-center">
                    Tercatat jam <b>${finalTime}</b>. Simpan dan kembali ke beranda?
                </p>
            </div>
        `,
        ...
        confirmButtonColor: isLate ? '#ef4444' : '#0ea5e9',
